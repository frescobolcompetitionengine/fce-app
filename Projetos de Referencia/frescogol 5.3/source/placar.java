...
public class placar extends PApplet {
...
public String ns (String str, int n) {
    int len = str.length();
...
public int conf_tempo () {
    return (ESQUENTA ? CONF_ESQUENTA : CONF_TEMPO);
}

public int conf_golpes (int jog) {
    if (CONF_TRINCA) {
        if (jog == LADO_PIVO) {
            return 0;
        } else {
...
public int conf_maximas (int jog) {
    if (CONF_TRINCA) {
        if (jog == LADO_PIVO) {
            return 0;
        } else {
...
public void go_terminando () {
    ESTADO = "terminando";
}

 public void go_termino () {
    ESTADO = "terminado";
    SNDS[3].play();
    ...
    try {
        ...
        pw.write (
            ...
            CONF_NOMES[0] + " ; " + JOGO_JOGS[0][IDX_PTS] + " ; " + JOGO_JOGS[0][IDX_GLP] + " ; " + (PApplet.parseFloat(JOGO_JOGS[0][IDX_MED])/100) + " ; " +
            CONF_NOMES[1] + " ; " + JOGO_JOGS[1][IDX_PTS] + " ; " + JOGO_JOGS[1][IDX_GLP] + " ; " + (PApplet.parseFloat(JOGO_JOGS[1][IDX_MED])/100) + " ; " +
            "\n"
        );
        pw.close();
    } catch (IOException e) {
...
public void _jogo_lado_ins (ArrayList<int[]> seq, IntList inss) {
...
public void _jogo_lado (int jog) {
    IntList kmhs = new IntList();
        ...
        for (int j=0; j<seq.size()-1; j++) {    // -1: ignora ultimo golpe
            ...
            if (kmh >= CONF_VEL_MIN) {
                ...
                if (CONF_MAXIMAS!=0 && j>0) { // precisa de golpe anterior
                    ...
                    if (prev1[IDX_NOW]+1000<golpe[IDX_NOW] || prev1[IDX_JOG]==jog || kmh2*1.2f<=kmh) {
                        if (golpe[IDX_BAK] == 0) {
                            nrms.append(kmh);
                        } else {
                            baks.append(kmh);
...
public int jogo_kmh (ArrayList<int[]> seq, int i) {
    int[] cur = seq.get(i);
    int kmh = cur[IDX_KMH];
...
public int jogo_quedas () {
    return JOGO_QUEDAS + JOGO_QUEDAS_MANUAL;
}

public int jogo_quedas_pct () {
...
int old = millis();
public int radar_mock () {
    int dt  = NOW - old;
    if (dt > RADAR_MOCK_SPEED) {
        old = NOW;
        if (random(0,4) <= 2) {
            int vel = PApplet.parseInt(random(30,CONF_VEL_MAX));
            return (PApplet.parseInt(random(0,5))>=2) ? vel : -vel;
        }
    }
    return 0;
}
...
public int four (byte[] s, int idx) {
    return
        (s[idx+0] - '0') * 1000 +
        (s[idx+1] - '0') *  100 +
        (s[idx+2] - '0') *   10 +
...
public boolean check_num (byte c) {
    return (c>='0' && c<='9');
}

public boolean radar_check (byte[] s) {
    return
...
public void sound (int kmh, int prv) {
//println(kmh);
    if (kmh < CONF_VEL_MIN) {
//println("min");
        if (!conf_radar()) {
...
public void trata_nome (int idx, String json) {
    ...
    } else if (PApplet.parseInt(key)>=PApplet.parseInt('a') && PApplet.parseInt(key)<=PApplet.parseInt('z') || PApplet.parseInt(key)>=PApplet.parseInt('A') && PApplet.parseInt(key)<=PApplet.parseInt('Z') || key=='_'){
        ...
        if (conf_radar()) {
            if (kmh > 1) {
                ...
                if (ESTADO_JOGANDO.equals("jogando")) {
                    ...
                    boolean back = (RADAR_MOCK && (PApplet.parseInt(random(5))==0)) ||
...
public void draw_debug (int kmh_) {
    ...
    if (cur > 0) {
        ...
        line(2.5f*W, 4*H, 2.5f*W+60*dy, 4*H);
        line(2.5f*W+60*dy, 4*H, 2.5f*W+45*dy, 4*H+20*dy);
        line(2.5f*W+60*dy, 4*H, 2.5f*W+45*dy, 4*H-20*dy);
    } else if (cur < 0) {
        //ellipse(8*W, 4*H, 60*dy, 60*dy);
        line(8.5f*W, 4*H, 8.5f*W-60*dy, 4*H);
        line(8.5f*W-60*dy, 4*H, 8.5f*W-45*dy, 4*H-20*dy);
        line(8.5f*W-60*dy, 4*H, 8.5f*W-45*dy, 4*H+20*dy);
    }
    //strokeWeight(1);
    noStroke();
...
public void draw_jogo () {
    ...
    // TEMPO
    {
        ...
        if (show>5 || ESTADO.equals("terminado") || blink_timer_state) {
            if (ESTADO.equals("terminado")) {
                ...
                text("00:00", width/2, 1.25f*H-10*dy);
            } else {
                text(mins+":"+segs, width/2, 1.25f*H-10*dy);
            }
        }
        ...
        if (ESQUENTA) {
            ...
            textSize(35*dy);
            textAlign(CENTER, CENTER);
            text("AQUECIMENTO", width/2, 2.50f*H);
        } else {
            ...
            textSize(35*dy);
            textAlign(CENTER, CENTER);
            text(abs(descanso) + " s", width/2, 2.50f*H);
            image(IMG_DESCANSO, width/2-50*dx, 2.50f*H);
            if (descanso < 0) {
                textSize(20*dy);
                text("(ESGOTADO)", width/2, 2.5f*H+30*dy);
            }
        }
    }
    ...
    // QUEDAS
    {
        fill(255, 0, 0);
        ellipseMode(CENTER);
        ellipse(width/2, height/2, 1.5f*H, 1.5f*H);
    ...
    if (ESTADO.equals("jogando")) {
                ...
                if (NOW <= golpe[IDX_NOW]+GOLPE_DELAY) {
                    ...
                    if (kmh == 0) {
                        ...
                        if (xxx >= 0) {
                            ...
                            if (jog == ONE) {
                                draw_ultima(1.5f*W, kmh);
                            } else {
                                draw_ultima(9.5f*W, kmh);
                            }
                        }
                    } else {
                        if (jog == ZER) {
        ...
        for (int i=0; i<2; i++) {
            draw_lado_medias(0.625f*W, ZER);
            draw_lado_medias(7.625f*W, ONE);
        }
    }
    ...
    if (CONF_TRINCA) {
        if (LADO_PIVO == 0) {
            image(IMG_RAQUETE, 4*W-20*dx,       2.5f*H);
            image(IMG_RAQUETE, 7*W+20*dx,       2.5f*H-10*dy);
            image(IMG_RAQUETE, 7*W+20*dx+10*dx, 2.5f*H+10*dy);
        } else {
            image(IMG_RAQUETE, 4*W-20*dx-10*dx, 2.5f*H-10*dy);
            image(IMG_RAQUETE, 4*W-20*dx,       2.5f*H+10*dy);
            image(IMG_RAQUETE, 7*W+20*dx,       2.5f*H);
        }
    }
...

    {
        ...
        if (ESQUENTA) {
            fill(200, 200, 0);
            ellipseMode(CENTER);
            ellipse(6.5f*W, 5.2f*H, 20*dy, 20*dy);
        } else if (RADAR_AUTO) {
            fill(0, 150, 0);
            ellipseMode(CENTER);
            ellipse(6.5f*W, 5.2f*H, 20*dy, 20*dy);
        ...
        fill(150,150,150);
        textSize(15*dy);
        float pct = PApplet.parseFloat(jogo_quedas_pct()) / 100;
        String conta = "(" + (ps[0]+ps[1]) + " - " + pct + "%)";
        text(conta, width/2, 7.5f*H+20*dy);
    }
}

public void draw_logo (float x, PImage img) {
    noStroke();
    fill(255);
    //rect(x, 0, 4*W, 2*H);
    image(img, x+2*W, H);
...
public void draw_nome (float x, int jog, boolean digitando) {
    ...
    textSize(90*dy);
    textAlign(CENTER, CENTER);
    text(nome, x+2*W, 2.25f*H-10*dy);
}

public void draw_ultima (float x, int kmh) {
    if (kmh == 0) {
        return;
    }
    if (kmh >= CONF_VEL_MIN) {
...
public void draw_lado (float x, int jog) {
    int[] JOG = JOGO_JOGS[jog];
    float X = 1.25f*W;

    image(IMG_GOLPES, x+W/2, 5.875f*H+ 5*dy);
    image(IMG_BAND,   x+W/2, 7.125f*H+10*dy);
    ...
    text(JOG[IDX_GLP], x+2*W, 5.875f*H);       // golpes
    fill(150,150,150);
    textSize(20*dy);
    float w1 = textWidth(str(JOG[IDX_GLP]));  // golpes
    textAlign(TOP, LEFT);
    text("/"+glps, x+2*W+w1+10*dx, 5.875f*H+30*dy);  // limite

    if (CONF_REGRA==5 && CONF_INTENSIDADE!=0) {
        textSize(20*dy);
        textAlign(TOP, LEFT);
        text("+"+JOG[IDX_INSS], x+2*W+w1+15*dx, 5.875f*H); // intensidade
    }
    ...
    if (CONF_REGRA==5 && CONF_MAXIMAS!=0) {
        ...
        for (int i=7; i<=8; i++) {
            ...
            textAlign(CENTER, CENTER);
            textSize(40*dy);
            float y = (i==7) ? 5.625f : 6.125f;
            if (JOG[i]>=atas) {         // backs vs limite
                fill(0,200,0);
            }
            int N = min(atas,JOG[i]);
            text(N, x+3.5f*W, y*H);      // golpes
            fill(150,150,150);
            textSize(15*dy);
            float w2 = textWidth(str(N)); // golpes
            textAlign(TOP, LEFT);
            text("/"+atas, x+3.5f*W+w2+10*dx, y*H+20*dy); // limite
        }
    }
    ...
    if (CONF_EQUILIBRIO != 0) {
        ...
        if (!ok) {
            ...
            textSize(30*dy);
            text("↓"+pct+"%", x+3.5f*W, 7.125f*H);
            textSize(65*dy);
        }
        fill(clr);
    }
    text(JOG[IDX_PTS], x+2*W, 7.125f*H);
}

public void draw_lado_medias (float x, int jog) {
    int[] JOG = JOGO_JOGS[jog];
    float X = 0.875f*W;
    float x1 = x + X;
    float x2 = x + X+X;
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "placar" };
    if (passedArgs != null) {
